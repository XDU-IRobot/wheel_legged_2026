...
定点锁定在松开摇杆时自动将底盘锚定在当前纵向位置，防止惯性漂移。核心思想：松开摇杆 → 速度目标归零 → **等待斜坡函数将 `filtered_s_dot` 平滑降至零** → 零速保持 N 个周期确认斜坡已走完 → 锁定并捕获当前位置为参考。退出时通过滞回双阈值防止边界抖动。

与旧方案（判断当前物理车速 < 阈值）的关键区别：新方案判断的是**斜坡输出值** `filtered_s_dot` 是否已归零。因为进入锁定前目标速度已经设为零，`filtered_s_dot` 归零说明斜坡函数已经走完——此时车身理应已停下，无需额外判断物理车速。这消除了 `kLockPointCaptureSpeedThresholdMps` 这一需要按变体手工标定的参数。
...
| `kLockPointMinDwellTicks` | 100 | 10 | 10 | 锁定/解锁切换后最小消抖 tick 数 |
| `kLockPointFilteredSdotZeroThreshold` | 1e-5 | 1e-5 | 1e-5 | filtered_s_dot 低于此值视为"已归零" |
| `kLockPointSpeedSettledTicks` | 50 | 30 | 30 | filtered_s_dot 维持零速的最小连续 tick 数（确认斜坡完成） |
...
摇杆归中 ──── target_s_dot 设为零 ──── filtered_s_dot 经斜坡逐步下降 ──── 到达零速 ────[保持 N ticks]─── 锁定
                                                                                            │
                                                                            kLockPointSpeedSettledTicks
                                                                            确认斜坡完成，捕获 s_ref
```

退出锁定：摇杆超过 ExitThreshold + 消抖 dwell → 立即退出，alpha 快速衰减。
...
**斜坡完成判据（核心变更）** — `filtered_s_dot` 是 `RampValueToTarget()` 的输出值。当摇杆归中（`request_lock == true`）时，速度目标已设为零，斜坡函数每周期以 brake_step 递减 `filtered_s_dot`，到达零时 clamp 到精确 0.0。代码跟踪 `filtered_s_dot` 保持在 `kLockPointFilteredSdotZeroThreshold`（1e-5，即浮点零）以下的连续周期数。连续保持 ≥ `kLockPointSpeedSettledTicks` 周期后，确认斜坡已完成，进入锁定并捕获当前位置 `s_ref`。

这样做的好处：
- 不依赖物理车速估计（估计器可能有延迟或噪声）
- 自动适配不同腿长的制动速率（斜坡参数本身已按腿长分档）
- 消除了需要按变体标定的 `kLockPointCaptureSpeedThresholdMps`

**位置捕获** — 与新方案配套，`lock_point_s_ref` 在进入锁定的瞬间捕获当前 `s`。此时斜坡已走完，车速为零，捕获的位置即为静止位置。
...
|---|---|---|---|
| 锁点消抖 Dwell | 200ms (100 ticks) | 20ms (10 ticks) | 20ms (10 ticks) |
| 锁点零速判稳 Ticks | 50 ticks (100ms) | 30 ticks (60ms) | 30 ticks (60ms) |
| 锁点 filtered_s_dot 零阈值 | 1e-5 | 1e-5 | 1e-5 |
