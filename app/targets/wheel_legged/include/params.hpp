...
constexpr std::uint16_t kAutoJumpRearmDistanceMm = 1100U;
constexpr float kPollRequestFrequencyHz = 50.0f;
...
// ==== 跳跃 ====
constexpr std::uint32_t kJumpLowPrepMs = 2U;        ///< 跳跃预备阶段持续时间 [ms]
constexpr std::uint32_t kJumpLowPushMaxMs = 2500U;  ///< 跳跃蹬伸阶段最长持续时间 [ms]
constexpr std::uint32_t kJumpLowRecoverMs = 600U;   ///< 跳跃回收阶段持续时间（保底超时）[ms]
...
// ==== 跳跃（PID 增益）====
constexpr float kJumpPushForceN = 310.0f;                                     ///< 蹬伸阶段单腿基础支撑力 [N]
