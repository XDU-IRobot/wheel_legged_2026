...
constexpr float kLargeTurnThresholdRad = 0.18f;  ///< 大转向检测阈值：motor_error 超此值触发先减速再转向 [rad]
constexpr float kSafeTurnSpeedMps = 0.1f;  ///< 大转向安全速度上限：超此速度时强制限速 [m/s]
constexpr float kLargeTurnThetaThresholdRad = 0.1f;  ///< 大转向腿摆角阈值：摆角超此值需先回正再转向 [rad]
constexpr float kLargeTurnRecoveryAccelScale = 0.85f;  ///< 大转向恢复加速缩放：限速解除后加速斜坡乘以该系数，越小越缓
constexpr float kVxInputDeadbandNorm = 0.05f;        ///< 前进输入死区
constexpr float kVyInputDeadbandNorm = 0.05f;        ///< 平移输入死区
