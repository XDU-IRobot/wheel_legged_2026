...
// ==== 基本运动（横滚平衡）====
constexpr float kRollBalanceTargetRad = -0.00159f;  ///< 横滚平衡目标角 [rad]
...
// ==== 基本运动（PID 增益）====
constexpr PidGains kLeftL0Pid{3000.0f, 0.f, 170.0f, 170.0f, 30.0f};   ///< 左腿腿长 PID（常规）
constexpr PidGains kRightL0Pid{3000.0f, 0.f, 170.0f, 170.0f, 30.0f};  ///< 右腿腿长 PID（常规）
constexpr PidGains kRollPid{1200.0f, 0.0f, 200.0f, 80.0f, 0.0f};      ///< 横滚平衡 PID
...
constexpr uint32_t kPositionHoldTimeoutTicks =
    1000U;  ///< 位置锚定超时 [ticks]（斜坡归零后最多等待此周期数，超时强制冻结）
constexpr float kPositionErrorScaleLowLeg = 2.5f;      ///< 低腿长位置误差缩放
constexpr float kVelocityErrorScaleLowLeg = 1.5f;      ///< 低腿长速度误差缩放
constexpr float kPositionErrorScaleMidLeg = 2.f;       ///< 中腿长位置误差缩放
constexpr float kVelocityErrorScaleMidLeg = 1.3f;      ///< 中腿长速度误差缩放
constexpr float kPositionErrorScaleHighLeg = 1.0f;     ///< 高腿长位置误差缩放
constexpr float kVelocityErrorScaleHighLeg = 2.0f;     ///< 高腿长速度误差缩放
...
constexpr float kExpectedThetaLrBiasRadLowLeg = 0.0f;      ///< 低腿长期望右腿摆角偏置 [rad]
constexpr float kExpectedThetaBBiasRad = -0.0f;            ///< 期望机体俯仰偏置 [rad]
constexpr float kExpectedDisplacementBiasMLowLeg = -0.9f;  ///< 低腿长期望位移偏置 [m]
