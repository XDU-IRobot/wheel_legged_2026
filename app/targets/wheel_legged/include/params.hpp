...
const DmMitSettings kPitchMotorSettings{0x17, 0x07, kPi, 30.f, 28.f, {0.f, 500.f}, {0.f, 5.f}};
...
enum class IdentSubMode : uint8_t {
  ...
  kFriction,      ///< Step 2: 低速摩擦 — Yaw/Pitch 分段匀速扫角
...
constexpr size_t kCouplingAngleMaxCount = 6;
}  // namespace gimbal_ident_common
...
constexpr float kIdentTheta[9] = {
    ...
    0, 0, 0, 0.421835f, 0.294483f, 0, 0, 0, 0};

/// @brief Pitch 力矩前馈常值偏置  [Nm]
// constexpr float kPitchFeedforwardBiasNm = -15.559206f;
constexpr float kPitchFeedforwardBiasNm = 5.2554f;
}  // namespace gimbal
...
/// @brief 摩擦模式: Yaw 速度环匀速转一圈 (2π)
constexpr float kFrictionVelocityRadS = 0.3f;       ///< 摩擦辨识 yaw 速度 [rad/s]
constexpr float kFrictionPitchVelocityRadS = 0.3f;  ///< 摩擦辨识 pitch 速度 [rad/s]
...
constexpr float kLeftSpringC0 = -124;
constexpr float kLeftSpringC1 = -80;
constexpr float kLeftSpringC2 = -943;
constexpr float kLeftSpringC3 = 2683;
// -- 右腿弹簧补偿三次多项式系数 --
constexpr float kRightSpringC0 = -124;
constexpr float kRightSpringC1 = -80;
constexpr float kRightSpringC2 = -943;
constexpr float kRightSpringC3 = 2683;
...
constexpr PidGains kLeftL0PidStandup{1500.0f, 0.f, 100.0f, 130.0f, 10.0f};   ///< 左腿腿长 PID（起立）
constexpr PidGains kRightL0PidStandup{1500.0f, 0.f, 100.0f, 130.0f, 10.0f};  ///< 右腿腿长 PID（起立）
...
constexpr float kYawFollowFixedTargetRad = 0.f;        ///< 偏航跟随固定目标偏置角 [rad]（前进方向）
...
constexpr PidGains kIdentYawPosPid{20.0f, 0.0f, 0.1f, 10.0f, 0.0f};    ///< 辨识模式 yaw 位置 PID
constexpr PidGains kIdentPitchPosPid{60.0f, 0.0f, 0.5f, 10.0f, 0.0f};  ///< 辨识模式 pitch 位置 PID
constexpr PidGains kIdentYawVelPid{3.0f, 15.0f, 0.0f, 10.0f, 2.0f};    ///< Friction step yaw 速度 PID
constexpr PidGains kIdentPitchVelPid{5.0f, 20.0f, 0.0f, 10.0f, 2.0f};  ///< Friction step pitch 速度 PID
...
// ── 分步辨识参数 ──
constexpr float kGravityPitchAngles[] = {-2.55f, -2.40f, -2.29f, -2.15f, -2.00f, -1.85f, -1.70f};
...
/// @brief 摩擦模式: Yaw 速度环匀速转一圈 (2π)
constexpr float kFrictionVelocityRadS = 0.05f;       ///< 摩擦辨识 yaw 速度 [rad/s]
constexpr float kFrictionPitchVelocityRadS = 0.02f;  ///< 摩擦辨识 pitch 速度 [rad/s]
...
constexpr size_t kCouplingAngleCount = 4;
constexpr float kCouplingFreqHz = 0.2f;
constexpr float kCouplingAmplitude = 0.5f;
constexpr float kCouplingDurationPerAngle = 5.0f;
}  // namespace gimbal_ident
...
// ==== 跳跃 ====
constexpr std::uint32_t kJumpLowPrepMs = 350U;      ///< 跳跃预备阶段持续时间 [ms]
...
constexpr PidGains kLeftL0PidJumpThree{1000.0f, 0.f, 220.0f, 120.0f, 30.0f};  ///< 左腿回收 PID
constexpr PidGains kRightL0PidJumpThree{1000.0f, 0.f, 220.0f, 120.0f, 30.0f};  ///< 右腿回收 PID
...
constexpr float kYawFollowFixedTargetRad = 2.444f;  ///< 偏航跟随固定目标偏置角 [rad]
...
constexpr PidGains kYawPositionPid{25.0f, 0.0f, 0.5f, 10.0f, 1.0f};    ///< 偏航位置 PID
constexpr PidGains kYawSpeedPid{0.8f, 0.0f, 0.0f, 6.0f, 0.4f};         ///< 偏航速度 PID
constexpr PidGains kPitchPositionPid{35.0f, 0.0f, 0.4f, 10.0f, 0.4f};  ///< 俯仰位置 PID
...
// -- 中腿长下压 --
constexpr float kMidLegDipTriggerLengthM = 0.25f;  ///< 中腿长模式下触发下压的腿长阈值 [m]
constexpr float kMidLegDipTargetLengthM = 0.22f;   ///< 下压目标腿长 [m]
constexpr uint16_t kMidLegDipHoldTicks = 1000;     ///< 下压维持时间 [ticks @ 500Hz = 1s]
...
// ==== 基本运动（PID 增益）====
constexpr PidGains kLeftL0Pid{3000.0f, 0.f, 200.0f, 170.0f, 30.0f};   ///< 左腿腿长 PID（常规）
constexpr PidGains kRightL0Pid{3000.0f, 0.f, 200.0f, 170.0f, 30.0f};  ///< 右腿腿长 PID（常规）
...
constexpr PidGains kLeftL0PidJumpTwo{6000.0f, 0.0f, 100.0f, 300.0f, 0.0f};    ///< 左腿蹬伸 PID
constexpr PidGains kRightL0PidJumpTwo{6000.0f, 0.0f, 100.0f, 300.0f, 0.0f};   ///< 右腿蹬伸 PID
...
// -- 云台启动归中判稳 --
constexpr float kGimbalStartupYawAlignErrorRad = 0.05f;            ///< 归中完成位置误差阈值 [rad]
constexpr float kGimbalStartupYawAlignVelRadS = 0.25f;            ///< 归中完成速度阈值 [rad/s]
constexpr std::uint32_t kGimbalStartupYawAlignStableTicks = 10U;  ///< 归中判稳所需连续满足周期数

// -- 偏航就绪判稳 --
constexpr float kYawFollowDriveReadyErrorRad = 0.05f;           ///< 偏航就绪位置误差阈值 [rad]
constexpr float kYawFollowDriveReadyVelRadS = 0.25f;            ///< 偏航就绪速度阈值 [rad/s]
constexpr std::uint32_t kYawFollowDriveReadyStableTicks = 50U;  ///< 偏航就绪判稳所需连续周期数
...
constexpr float kLargeTurnThresholdRad = 0.3f;  ///< 大转向检测阈值：motor_error 超此值触发先减速再转向 [rad]
...
constexpr uint32_t kPositionHoldTimeoutTicks = 1000U;  ///< 位置锚定超时 [ticks]（斜坡归零后最多等待此周期数，超时强制冻结）
...
constexpr float kSpinThetaLrBiasRad = -0.15f;              ///< 小陀螺时右腿摆角偏置 [rad]
constexpr float kSpinLegLengthBiasM = 0.0f;             ///< 小陀螺时腿长偏差（左+右-）[m]
