...
constexpr float kPhiErrorScaleLowLeg = 1.f;            ///< 低腿长偏航角误差缩放
constexpr float kPhiErrorScaleMidLeg = 1.f;            ///< 中腿长偏航角误差缩放
constexpr float kPhiErrorScaleHighLeg = 1.f;           ///< 高腿长偏航角误差缩放
constexpr float kPhiDotErrorScaleLowLeg = 1.f;         ///< 低腿长偏航角速度误差缩放
constexpr float kPhiDotErrorScaleMidLeg = 1.f;         ///< 中腿长偏航角速度误差缩放
constexpr float kPhiDotErrorScaleHighLeg = 1.f;        ///< 高腿长偏航角速度误差缩放
constexpr float kThetaLlErrorScaleLowLeg = 1.f;        ///< 低腿长左腿摆角误差缩放
constexpr float kThetaLlErrorScaleMidLeg = 1.f;        ///< 中腿长左腿摆角误差缩放
constexpr float kThetaLlErrorScaleHighLeg = 1.f;       ///< 高腿长左腿摆角误差缩放
constexpr float kThetaLlDotErrorScaleLowLeg = 1.f;     ///< 低腿长左腿摆角速度误差缩放
constexpr float kThetaLlDotErrorScaleMidLeg = 1.f;     ///< 中腿长左腿摆角速度误差缩放
constexpr float kThetaLlDotErrorScaleHighLeg = 1.f;    ///< 高腿长左腿摆角速度误差缩放
constexpr float kThetaLrErrorScaleLowLeg = 1.f;        ///< 低腿长右腿摆角误差缩放
constexpr float kThetaLrErrorScaleMidLeg = 1.f;        ///< 中腿长右腿摆角误差缩放
constexpr float kThetaLrErrorScaleHighLeg = 1.f;       ///< 高腿长右腿摆角误差缩放
constexpr float kThetaLrDotErrorScaleLowLeg = 1.f;     ///< 低腿长右腿摆角速度误差缩放
constexpr float kThetaLrDotErrorScaleMidLeg = 1.f;     ///< 中腿长右腿摆角速度误差缩放
constexpr float kThetaLrDotErrorScaleHighLeg = 1.f;    ///< 高腿长右腿摆角速度误差缩放
constexpr float kThetaBErrorScaleLowLeg = 1.0f;        ///< 低腿长机体俯仰角误差缩放
constexpr float kThetaBErrorScaleMidLeg = 1.0f;        ///< 中腿长机体俯仰角误差缩放
constexpr float kThetaBErrorScaleHighLeg = 1.0f;       ///< 高腿长机体俯仰角误差缩放
constexpr float kThetaBDotErrorScaleLowLeg = 1.0f;     ///< 低腿长机体俯仰角速度误差缩放
constexpr float kThetaBDotErrorScaleMidLeg = 1.0f;     ///< 中腿长机体俯仰角速度误差缩放
constexpr float kThetaBDotErrorScaleHighLeg = 1.0f;    ///< 高腿长机体俯仰角速度误差缩放
...
constexpr float kFrictionYawVelocitiesRadS[] = {0.05, 0.10, 0.20, 0.30};
constexpr size_t kFrictionYawVelocityCount = sizeof(kFrictionYawVelocitiesRadS) / sizeof(kFrictionYawVelocitiesRadS[0]);
constexpr float kFrictionYawTravelRad = 6.28f;  // ≈2π, 接近 360°
constexpr float kFrictionPitchVelocitiesRadS[] = {0.05, 0.10, 0.20, 0.30};
constexpr size_t kFrictionPitchVelocityCount =
    sizeof(kFrictionPitchVelocitiesRadS) / sizeof(kFrictionPitchVelocitiesRadS[0]);
...
// ==== 姿态安全/倒地恢复 ====
constexpr float kStandupThetaThresholdRad = 1.2f;     ///< 起立完成判定 [rad]
constexpr float kStandupPhase0ThetaTargetRad = 1.5f;  ///< 起立 Phase 0/1 腿摆角目标 [rad]
constexpr float kStandupPhase1TargetLengthM = 0.1f;   ///< 起立 Phase 1 目标腿长 [m]
constexpr float kStandupPhase1ThetaTolRad = 0.6f;  ///< 起立 Phase 1 完成判定：摆角与目标差值容许 [rad]
...
constexpr float kRecoveryDecelZoneRad = 0.6f;   ///< 恢复减速区宽度 [rad]（接近目标边界时开始减速）
constexpr float kRecoveryMinSpeedRadS = 0.08f;  ///< 恢复减速区边界最低速度 [rad/s]
constexpr float kRecoveryGravityRampScale = 0.7f;     ///< 恢复时重力补偿斜坡比例（越大身体越不砸）
constexpr float kRecoveryPitchFeedforwardKp = 25.0f;  ///< 倒地恢复俯仰角前馈系数
constexpr float kRecoveryRollFeedforwardKp = 6.0f;    ///< 倒地恢复横滚角前馈系数
...
// ==== 基本运动（横滚平衡）====
constexpr float kRollBalanceTargetRad = 0.0134f;  ///< 横滚平衡目标角 [rad]

// ==== 基本运动（LQR 增益矩阵）====
static constexpr const auto &kCtrlPLow = ::wheel_legged::params::generated::kCtrlP;

// ==== 基本运动（PID 增益）====
constexpr PidGains kLeftL0Pid{3000.0f, 0.f, 150.0f, 170.0f, 30.0f};   ///< 左腿腿长 PID（常规）
constexpr PidGains kRightL0Pid{3000.0f, 0.f, 150.0f, 170.0f, 30.0f};  ///< 右腿腿长 PID（常规）
constexpr PidGains kRollPid{1000.0f, 0.0f, 200.0f, 80.0f, 0.0f};      ///< 横滚平衡 PID
...
// -- 云台启动归中判稳 --
constexpr float kGimbalStartupYawAlignErrorRad = 0.12f;           ///< 归中完成位置误差阈值 [rad]
constexpr float kGimbalStartupYawAlignVelRadS = 0.25f;            ///< 归中完成速度阈值 [rad/s]
constexpr std::uint32_t kGimbalStartupYawAlignStableTicks = 10U;  ///< 归中判稳所需连续满足周期数

// -- 偏航就绪判稳 --
constexpr float kYawFollowDriveReadyErrorRad = 0.12f;           ///< 偏航就绪位置误差阈值 [rad]
constexpr float kYawFollowDriveReadyVelRadS = 0.25f;            ///< 偏航就绪速度阈值 [rad/s]
constexpr std::uint32_t kYawFollowDriveReadyStableTicks = 50U;  ///< 偏航就绪判稳所需连续周期数
...
constexpr float kMaxSafeYawRateRadS = 3.5f;               ///< 摩擦圆最大安全偏航速率 [rad/s]
constexpr float kLargeTurnThresholdRad = 0.35f;  ///< 大转向检测阈值：motor_error 超此值触发先减速再转向 [rad]
constexpr float kSafeTurnSpeedMps = 0.3f;  ///< 大转向安全速度上限：超此速度时强制限速 [m/s]
constexpr float kLargeTurnThetaThresholdRad = 0.2f;  ///< 大转向腿摆角阈值：摆角超此值需先回正再转向 [rad]
constexpr float kLargeTurnRecoveryAccelScale = 1.f;  ///< 大转向恢复加速缩放：限速解除后加速斜坡乘以该系数，越小越缓
constexpr float kVxInputDeadbandNorm = 0.05f;        ///< 前进输入死区
constexpr float kVyInputDeadbandNorm = 0.05f;        ///< 平移输入死区
...
constexpr float kPositionErrorScaleHighLeg = 1.0f;     ///< 高腿长位置误差缩放
constexpr float kVelocityErrorScaleHighLeg = 1.0f;     ///< 高腿长速度误差缩放
constexpr float kPhiErrorScaleLowLeg = 1.3f;           ///< 低腿长偏航角误差缩放
constexpr float kPhiErrorScaleMidLeg = 1.f;            ///< 中腿长偏航角误差缩放
constexpr float kPhiErrorScaleHighLeg = 1.f;           ///< 高腿长偏航角误差缩放
constexpr float kPhiDotErrorScaleLowLeg = 1.15f;       ///< 低腿长偏航角速度误差缩放
constexpr float kPhiDotErrorScaleMidLeg = 1.f;         ///< 中腿长偏航角速度误差缩放
constexpr float kPhiDotErrorScaleHighLeg = 1.f;        ///< 高腿长偏航角速度误差缩放
constexpr float kThetaLlErrorScaleLowLeg = 1.3f;       ///< 低腿长左腿摆角误差缩放
constexpr float kThetaLlErrorScaleMidLeg = 1.f;        ///< 中腿长左腿摆角误差缩放
constexpr float kThetaLlErrorScaleHighLeg = 1.f;       ///< 高腿长左腿摆角误差缩放
constexpr float kThetaLlDotErrorScaleLowLeg = 1.15f;   ///< 低腿长左腿摆角速度误差缩放
constexpr float kThetaLlDotErrorScaleMidLeg = 1.f;     ///< 中腿长左腿摆角速度误差缩放
constexpr float kThetaLlDotErrorScaleHighLeg = 1.f;    ///< 高腿长左腿摆角速度误差缩放
constexpr float kThetaLrErrorScaleLowLeg = 1.3f;       ///< 低腿长右腿摆角误差缩放
constexpr float kThetaLrErrorScaleMidLeg = 1.f;        ///< 中腿长右腿摆角误差缩放
constexpr float kThetaLrErrorScaleHighLeg = 1.f;       ///< 高腿长右腿摆角误差缩放
constexpr float kThetaLrDotErrorScaleLowLeg = 1.15f;   ///< 低腿长右腿摆角速度误差缩放
constexpr float kThetaLrDotErrorScaleMidLeg = 1.f;     ///< 中腿长右腿摆角速度误差缩放
constexpr float kThetaLrDotErrorScaleHighLeg = 1.f;    ///< 高腿长右腿摆角速度误差缩放
constexpr float kThetaBErrorScaleLowLeg = 1.0f;        ///< 低腿长机体俯仰角误差缩放
constexpr float kThetaBErrorScaleMidLeg = 1.0f;        ///< 中腿长机体俯仰角误差缩放
...
constexpr float kThetaBDotErrorScaleMidLeg = 1.0f;     ///< 中腿长机体俯仰角速度误差缩放
constexpr float kThetaBDotErrorScaleHighLeg = 1.0f;    ///< 高腿长机体俯仰角速度误差缩放
constexpr float kYawFollowFixedTargetRad = 0.7941f;    ///< 偏航跟随固定目标偏置角 [rad]
...
// ==== 速度斜坡参数 ====
constexpr SdotRampParams kSdotRampLowLeg{0.006f, 0.006f};     ///< 低腿长速度斜坡
...
// constexpr float kSpinTargetYawDotRadS3 = -10.5f;         ///< 小陀螺目标自旋角速度 [rad/s] 65-75W
// constexpr float kSpinTargetYawDotRadS4 = -11.5f;         ///< 小陀螺目标自旋角速度 [rad/s] >75W
constexpr float kSpinTargetYawDotRadS1 = -11.5f;       ///< 小陀螺目标自旋角速度 [rad/s] ≤55W
constexpr float kSpinTargetYawDotRadS2 = -11.5f;       ///< 小陀螺目标自旋角速度 [rad/s] 55-65W
constexpr float kSpinTargetYawDotRadS3 = -11.5f;       ///< 小陀螺目标自旋角速度 [rad/s] 65-75W
constexpr float kSpinTargetYawDotRadS4 = -11.5f;       ///< 小陀螺目标自旋角速度 [rad/s] >75W
constexpr float kSpinTargetYawDotRadNoScS1 = -7.0f;    ///< 无超电小陀螺目标自旋角速度 [rad/s] ≤55W
constexpr float kSpinTargetYawDotRadNoScS2 = -8.0f;    ///< 无超电小陀螺目标自旋角速度 [rad/s] 55-65W
constexpr float kSpinTargetYawDotRadNoScS3 = -9.0f;    ///< 无超电小陀螺目标自旋角速度 [rad/s] 65-75W
constexpr float kSpinTargetYawDotRadNoScS4 = -10.f;    ///< 无超电小陀螺目标自旋角速度 [rad/s] >75W
constexpr float kSpinExitYawAlignThresholdRad = 0.2f;  ///< 小陀螺预测退出：yaw 对齐阈值 [rad]
constexpr float kSpinTranslationGain = 0.25f;          ///< 小陀螺平移增益
constexpr float kSpinThetaLlBiasRad = 0.f;             ///< 小陀螺时左腿摆角偏置 [rad]
constexpr float kSpinThetaLrBiasRad = -0.05f;          ///< 小陀螺时右腿摆角偏置 [rad]
constexpr float kSpinLegLengthBiasM = -0.005f;         ///< 小陀螺时腿长偏差（左+右-）[m]
...
namespace state_estimator {
constexpr float kDefaultDtS = 0.002f;                   ///< 估计器默认周期 [s]
constexpr float kDefaultExpectedSdotMps = 0.0f;         ///< 默认期望纵向速度 [m/s]
constexpr float kLegL1M = 0.215f;                       ///< 五连杆主动杆长度 [m]
constexpr float kLegL2M = 0.254f;                       ///< 五连杆从动杆长度 [m]
constexpr float kWheelRadiusM = 0.0575f;                ///< 驱动轮半径 [m]
constexpr float kWheelReductionRatio = 17.0f / 268.0f;  ///< 轮电机到车轮的速度换算比
