...
constexpr float kIdentTheta[9] = {
    ...
    0, 0, 0, 0.084498f, 0.183350f, 0, 0, 0, 0};

/// @brief Pitch 力矩前馈常值偏置  [Nm]
// constexpr float kPitchFeedforwardBiasNm = -15.559206f;
constexpr float kPitchFeedforwardBiasNm = 2.7410f;
}  // namespace gimbal
...
constexpr PidGains kIdentPitchVelPid{2.0f, 10.0f, 0.f, 10.0f, 10.0f};  ///< Friction step pitch 速度 PID
constexpr float kIdentPitchCenter = 0.6f;  ///< 辨识轨迹 pitch 中心角 [rad]（机械中位，实际需根据云台标定）
constexpr float kIdentPitchTopLimit = 0.4f;     ///< 辨识轨迹 pitch 下限 [rad]
constexpr float kIdentPitchBottomLimit = 1.3f;  ///< 辨识轨迹 pitch 上限 [rad]

// ── 分步辨识参数 ──
/// @brief 重力模式: Pitch 目标角度序列 [rad]
constexpr float kGravityPitchAngles[] = {0.5f, 0.6f, 0.7f, 0.8f, 0.9f, 1.f, 1.1f, 1.2f};
constexpr size_t kGravityAngleCount = 8;
...
constexpr float kFrictionPitchVelocityRadS = 0.1f;  ///< 摩擦辨识 pitch 速度 [rad/s]
...
constexpr float kLeftSpringC0 = -17.5763f;
constexpr float kLeftSpringC1 = -131.9713f;
constexpr float kLeftSpringC2 = -2831.1839f;
constexpr float kLeftSpringC3 = 6585.8279f;
// -- 右腿弹簧补偿三次多项式系数 --
constexpr float kRightSpringC0 = -17.5763f;
constexpr float kRightSpringC1 = -131.9713f;
constexpr float kRightSpringC2 = -2831.1839f;
constexpr float kRightSpringC3 = 6585.8279f;
...
constexpr float kMidLegDipTargetLengthM = 0.14f;   ///< 下压目标腿长 [m]
