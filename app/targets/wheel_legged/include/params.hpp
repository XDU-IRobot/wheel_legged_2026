...
constexpr float kStandupPhase1ThetaTolRad = 0.1f;      ///< 起立 Phase 1 完成判定：摆角与目标差值容许 [rad]
...
// constexpr PidGains kLeftL0Pid{2000.0f, 0.f, 100.0f, 170.0f, 10.0f};   ///< 左腿腿长 PID（常规）
// constexpr PidGains kRightL0Pid{2000.0f, 0.f, 100.0f, 170.0f, 10.0f};  ///< 右腿腿长 PID（常规）
...
// ==== 倒地自启（腿摆速度 PID）====
constexpr PidGains kLeftLegTurnPid{18.0f, 0.f, 0.0f, 13.0f, 10.0f};             ///< 左腿摆角速度 PID
constexpr PidGains kRightLegTurnPid{18.0f, 0.f, 0.0f, 13.0f, 10.0f};            ///< 右腿摆角速度 PID
...
constexpr float kExpectedDisplacementBiasMLowLeg = 0.27f;    ///< 低腿长期望位移偏置 [m]
...
constexpr float kGimbalStartupYawAlignVelRadS = 0.5f;            ///< 归中完成速度阈值 [rad/s]
constexpr std::uint32_t kGimbalStartupYawAlignStableTicks = 30U;  ///< 归中判稳所需连续满足周期数

// -- 偏航就绪判稳 --
constexpr float kYawFollowDriveReadyErrorRad = 0.1f;           ///< 偏航就绪位置误差阈值 [rad]
constexpr float kYawFollowDriveReadyVelRadS = 0.5f;            ///< 偏航就绪速度阈值 [rad/s]
constexpr std::uint32_t kYawFollowDriveReadyStableTicks = 30U;  ///< 偏航就绪判稳所需连续周期数
