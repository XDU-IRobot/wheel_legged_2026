...
class Gimbal2DofDynamics {
 public:
  static constexpr float kFrictionTransitionVelocityRadS = 0.02f;
 ...
 private:
  Eigen::Matrix<float, 2, 9> ComputeY(float q1, float q2, float dq1, float dq2, float ddq1, float ddq2,
    ...
    Y(0, 6) = std::tanh(dq1 / kFrictionTransitionVelocityRadS);  // theta_6: fc1 库仑摩擦
    ...
    Y(1, 8) = std::tanh(dq2 / kFrictionTransitionVelocityRadS);  // theta_8: fc2
