...
#define AI_MATH_RELU_GENERIC_NO_MAX(x, thr, alpha) AI_MATH_RELU_TEST(x, thr, ((alpha) * ((x) - (thr))), x)

#define AI_MATH_RELU_THRESHOLDED(x, thr) AI_MATH_RELU_TEST(x, thr, 0, (x))

#define AI_MATH_LEAKY_RELU(x, neg_slope, pos_slope) AI_MATH_RELU_TEST(x, 0, (x) * (neg_slope), (x) * (pos_slope))
...
#define AI_MATH_LOGISTIC(x) \
  (x < 0) ? (1.0f - (1.0f / (1.0f + AI_MATH_EXP(-AI_ABS(x))))) : (1.0f / (1.0f + AI_MATH_EXP(-AI_ABS(x))))
...
AI_DECLARE_STATIC
ai_vec4_float _get_vec4_float(const ai_handle fptr) { return *((const ai_vec4_float*)fptr); }
 ...
 */
AI_INTERFACE_ENTRY
void st_mat_init_f32(ai_matrix_f32* S, const uint16_t nRows, const uint16_t nColumns, float* pData);
 ...
 */
AI_INTERFACE_ENTRY
uint32_t st_mat_gemm_f32(const ai_matrix_f32* pSrcA, const ai_matrix_f32* pSrcB, const ai_matrix_f32* pSrcC,
                         const float alpha, const float beta, const int8_t tA, const int8_t tB, ai_matrix_f32* pDstY);
