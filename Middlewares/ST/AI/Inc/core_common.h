...
#define AI_NODE_COMMON_INIT(type_, id_, flags_, klass_, network_, next_, forward_)                     \
  .type = AI_NODE_TYPE(type_), .id = AI_ID_OBJ(id_), .flags = (flags_), .klass = AI_KLASS_OBJ(klass_), \
  .network = AI_NETWORK_OBJ(network_), .next = AI_NODE_OBJ(next_), .forward = AI_NODE_FUNC(forward_)
...
#define AI_FOR_EACH_TENSOR_LIST_DO(idx_, t_ptr_, tlist_ptr_)                                                     \
  ...
  for (ai_size idx_ = 0;                                                                                         \
       (idx_ < GET_TENSOR_LIST_SIZE(tlist_ptr_)) && ((t_ptr_ = GET_TENSOR_LIST_ITEM(tlist_ptr_, idx_)) != NULL); \
       ++idx_)
...
#define GET_TENSOR_LIST(chain_, type_) \
  ((AI_CONCAT(AI_TENSOR_CHAIN_, type_) < (chain_)->size) ? &(chain_)->chain[AI_CONCAT(AI_TENSOR_CHAIN_, type_)] : NULL)
...
#define GET_TENSOR_WEIGHTS(chain_, pos_) (GET_TENSOR_LIST_ITEM(GET_TENSOR_LIST_WEIGTHS(chain_), (pos_)))

#define GET_TENSOR_SCRATCH(chain_, pos_) (GET_TENSOR_LIST_ITEM(GET_TENSOR_LIST_SCRATCH(chain_), (pos_)))
 ...
 */
typedef AI_ALIGNED_TYPE(struct, 4) ai_node_stateful_s{AI_NODE_STATEFUL_FIELDS_DECLARE} ai_node_stateful;
