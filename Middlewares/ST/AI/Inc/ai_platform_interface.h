...
#define AI_ERROR_TRAP(net_, type_, code_) \
  ai_platform_network_set_error(AI_CONTEXT_OBJ(net_), AI_CONCAT(AI_ERROR_, type_), AI_CONCAT(AI_ERROR_CODE_, code_))
...
#define AI_SHAPE_INIT(dim_, ...) \
  AI_STORAGE_KLASS_INIT(AI_STORAGE_KLASS_SHAPE, dim_, AI_STORAGE_KLASS_PACK(ai_shape_dimension, dim_, ##__VA_ARGS__))

#define AI_SHAPE_INIT_FROM_BUFFER(dim_, buffer_) AI_STORAGE_KLASS_INIT(AI_STORAGE_KLASS_SHAPE, dim_, buffer_)
...
#define AI_STRIDE_INIT(dim_, ...) \
  AI_STORAGE_KLASS_INIT(AI_STORAGE_KLASS_STRIDE, dim_, AI_STORAGE_KLASS_PACK(ai_stride_dimension, dim_, ##__VA_ARGS__))

#define AI_STRIDE_INIT_FROM_BUFFER(dim_, buffer_) AI_STORAGE_KLASS_INIT(AI_STORAGE_KLASS_STRIDE, dim_, buffer_)
...
#define AI_TENSOR_INFO_OBJ_INIT(id_, flags_, data_size_) {.id = (id_), .flags = (flags_), .data_size = (data_size_)}
...
#define AI_TENSOR_OBJ_DECLARE(name_, attr_, id_, flags_, shape_, stride_, arrays_size_, arrays_ptr_, klass_obj_) \
  AI_ALIGNED(4)                                                                                                  \
  attr_ ai_tensor name_ = AI_TENSOR_OBJ_INIT(id_, flags_, AI_PACK(shape_), AI_PACK(stride_), arrays_size_,       \
                                             AI_PACK(arrays_ptr_), AI_PACK(klass_obj_));

/********************************* TENSOR STATE MACROS  ***********************/
#define AI_TENSOR_STATE_OBJ_INIT(end_ptr_, curr_ptr_, stride_, size_) {(end_ptr_), (curr_ptr_), (stride_), (size_)}
...
AI_DEPRECATED
#define AI_TENSOR_LIST_ENTRY(...) AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, AI_NUMARGS(__VA_ARGS__), __VA_ARGS__)

AI_DEPRECATED
#define AI_TENSOR_LIST_IO_ENTRY(flags_, size_, ...) AI_TENSOR_LIST_IO_OBJ_INIT(flags_, size_, __VA_ARGS__)
...
#define AI_TENSOR_LIST_OBJ_EMPTY {.size = 0, .flags = AI_FLAG_NONE, .tensor = (ai_tensor*[]){NULL}, .info = NULL}
...
#define AI_TENSOR_LIST_IO_OBJ_INIT(flags_, size_, ...)                                                            \
  {                                                                                                               \
    .size = (size_), .flags = (flags_), .tensor = (ai_tensor*[]){__VA_ARGS__}, .info = (ai_tensor_list_info[1]) { \
      {                                                                                                           \
        .buffer = (ai_buffer[size_]){AI_STRUCT_INIT}, .state = (ai_tensor_state[size_]){AI_STRUCT_INIT},          \
...
#define AI_TENSOR_CHAIN_IO_OBJ_DECLARE(name_, attr_, flags_, in_tensor_list_, out_tensor_list_) \
  AI_ALIGNED(4)                                                                                 \
  attr_ ai_tensor_chain_io name_ = AI_TENSOR_CHAIN_IO_OBJ_INIT(flags_, in_tensor_list_, out_tensor_list_);
...
AI_DEPRECATED
#define AI_NETWORK_OBJ_INIT(weights_buffer_, activations_buffer_, in_tensor_list_ptr_, out_tensor_list_ptr_,       \
  ...
  {                                                                                                                \
      ...
      .batch_id = 0,                                                                                               \
      .buffers =                                                                                                   \
          AI_NETWORK_BUFFERS_INIT(AI_BUFFER_ARRAY_OBJ_INIT_STATIC(AI_FLAG_NONE, 1, AI_PACK(weights_buffer_)),      \
                                  AI_BUFFER_ARRAY_OBJ_INIT_STATIC(AI_FLAG_NONE, 1, AI_PACK(activations_buffer_))), \
      .tensors =                                                                                                   \
          AI_TENSOR_CHAIN_IO_OBJ_INIT(AI_FLAG_NONE, AI_PACK(in_tensor_list_ptr_), AI_PACK(out_tensor_list_ptr_)),  \
...
#define AI_NETWORK_OBJ_INIT(weights_buffer_, activations_buffer_, in_tensor_list_ptr_, out_tensor_list_ptr_,      \
  ...
  {                                                                                                               \
      ...
      .n_batches = 0,                                                                                             \
      .batch_id = 0,                                                                                              \
      .buffers = AI_NETWORK_BUFFERS_INIT(AI_PACK(weights_buffer_), AI_PACK(activations_buffer_)),                 \
      .tensors =                                                                                                  \
          AI_TENSOR_CHAIN_IO_OBJ_INIT(AI_FLAG_NONE, AI_PACK(in_tensor_list_ptr_), AI_PACK(out_tensor_list_ptr_)), \
      .input_node = AI_NODE_OBJ(in_node_ptr_),                                                                    \
      .current_node = AI_NODE_OBJ(NULL),                                                                          \
...
#define AI_NETWORK_OBJ_DECLARE(name_, attr_, weights_buffer_, activations_buffer_, in_tensor_list_ptr_,         \
                               out_tensor_list_ptr_, in_node_ptr_, signature_, klass_obj_)                      \
  AI_ALIGNED(4)                                                                                                 \
  attr_ ai_network name_ =                                                                                      \
      AI_NETWORK_OBJ_INIT(AI_PACK(weights_buffer_), AI_PACK(activations_buffer_), AI_PACK(in_tensor_list_ptr_), \
...
#define AI_ANY_PTR_INIT(ptr_) {.handle = (ai_handle)(ptr_)}
...
typedef ai_u32 (*ai_observer_node_cb)(const ai_handle cookie, const ai_u32 flags, const ai_observer_node* node);
...
typedef ai_u32 (*ai_node_exec_cb)(const ai_node_exec_state state, struct ai_node_s* cur, const ai_handle ctx);
 ...
 */
AI_INTERFACE_TYPE
ai_bool ai_platform_get_weights_map(ai_ptr* map, const ai_size map_size, const ai_network_params* params);
 ...
 */
AI_INTERFACE_TYPE
ai_bool ai_platform_get_activations_map(ai_ptr* map, const ai_size map_size, const ai_network_params* params);
 ...
 */
AI_INTERFACE_TYPE
ai_bool ai_platform_bind_network_params(ai_network_params* params, const ai_buffer_array* map_weights,
 ...
 */
AI_INTERFACE_TYPE
ai_bool ai_platform_network_set_error(ai_context* net_ctx, const ai_error_type type, const ai_error_code code);
 ...
 */
AI_INTERFACE_TYPE
ai_error ai_platform_network_create(ai_handle* network, const ai_buffer* network_config, ai_context* net_ctx,
...
AI_INTERFACE_TYPE
ai_buffer* ai_platform_stai_bind_io(ai_u16* n_buffer, ai_buffer* dst_io_buffers, const stai_ptr* src_io_buffers,
...
ai_bool ai_platform_stai_update_io(ai_i32* batch_id, stai_ptr* dst_inputs, stai_ptr* dst_outputs,
                                   const ai_buffer* src_inputs, const ai_buffer* src_outputs, const ai_size n_inputs,
                                   const ai_size n_outputs);
 ...
 */
AI_INTERFACE_TYPE
ai_bool ai_platform_observer_register(ai_handle network, ai_observer_node_cb cb, ai_handle cookie, ai_u32 flags);
 ...
 */
AI_INTERFACE_TYPE
ai_bool ai_platform_observer_unregister(ai_handle network, ai_observer_node_cb cb, ai_handle cookie);
