...
#define CORE_GET_BUFFER_META_INFO(meta_info_, tensor_ptr_) core_get_buffer_meta_info(meta_info_, tensor_ptr_)
...
#define CORE_MEMORY_OVERLAP_INIT(partial_, range_, chain_id_, tensor_id_) \
  {.partial = (partial_), .range = AI_PACK(range_), .chain_id = (chain_id_), .tensor_id = (tensor_id_)}

#define CORE_OFFSET(offset_, max_) ((ai_i32)(((offset_) < 0) ? AI_MAX((max_) - (offset_), 0) : AI_MIN(offset_, max_)))
...
#define AI_TENSOR_INTEGER_GET_SCALE(t_, idx_) AI_INTQ_INFO_LIST_SCALE(AI_KLASS_GET_INTQ_INFO_LIST(t_), ai_float, idx_)
...
#define AI_FOR_EACH_BUFFER_ARRAY_ITEM(buffer_ptr_, buffer_array_ptr_, start_pos_, end_pos_)                           \
  ai_buffer* buffer_ptr_ =                                                                                            \
      AI_BUFFER_ARRAY_ITEM(buffer_array_ptr_, CORE_OFFSET(end_pos_, AI_BUFFER_ARRAY_SIZE(buffer_array_ptr_)));        \
  for (; buffer_ptr_ && AI_BUFFER_ARRAY_SIZE(buffer_array_ptr_) &&                                                    \
         (buffer_ptr_ >=                                                                                              \
          AI_BUFFER_ARRAY_ITEM(buffer_array_ptr_, CORE_OFFSET(start_pos_, AI_BUFFER_ARRAY_SIZE(buffer_array_ptr_)))); \
       buffer_ptr_--)
...
#define AI_ARRAY_OBJ_DATA_SIZE(array_) AI_ARRAY_GET_DATA_BYTE_SIZE(AI_ARRAY_OBJ_FMT(array_), AI_ARRAY_OBJ_SIZE(array_))
...
/*****************************************************************************/
#define AI_NODE_IS_FIRST(node) (AI_NODE_OBJ(node) == AI_NODE_OBJ(AI_NODE_OBJ(node)->network->input_node))

#define AI_NODE_IS_LAST(node_) ((AI_NODE_OBJ(node_) == AI_NODE_OBJ(node_)->next) || (AI_NODE_OBJ(node_)->next == NULL))

#define AI_FOR_EACH_NODE_DO(node_, nodes_) \
  for (ai_node* node_ = AI_NODE_OBJ(nodes_); (node_); node_ = ((AI_NODE_IS_LAST(node_)) ? NULL : (node_)->next))
...
ai_address_range core_address_range_init(const ai_handle start, const ai_handle end) {
  ...
  r.size = (ai_size)((start < end) ? ((ai_uptr)end - (ai_uptr)start) : ((ai_uptr)start - (ai_uptr)end));
  return r;
}
