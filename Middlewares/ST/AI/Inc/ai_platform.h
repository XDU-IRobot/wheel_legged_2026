...
#define AI_PLATFORM_API_VERSION AI_VERSION(AI_PLATFORM_API_MAJOR, AI_PLATFORM_API_MINOR, AI_PLATFORM_API_MICRO)
...
#define AI_CUSTOM_SIZE(type) \
  (ai_custom_type_signature)((AI_IS_UNSIGNED(type)) ? (0x80 | (sizeof(type) & 0x7f)) : (sizeof(type) & 0x7f))
...
#define AI_NETWORK_PARAMS_INIT(params_, activations_) {.params = params_, .activations = activations_}
...
#define AI_PBITS_GET_WORD(word_ptr_, bits_) (((ai_pbits*)(word_ptr_)) + ((bits_) >> AI_PBITS_SHIFTS))
...
#define AI_INTQ_INFO_LIST_SCALE(list_, type_, pos_) \
  (((list_) && (list_)->info && ((pos_) < (list_)->size)) ? ((type_*)((list_)->info->scale))[(pos_)] : 1.0f)
...
#define AI_BUFFER_FMT_SET(type_id_, sign_bit_, float_bit_, bits_, fbits_)                     \
  AI_BUFFER_FMT_OBJ(AI_BUFFER_FMT_PACK(0, 0x1, 24) | AI_BUFFER_FMT_PACK(sign_bit_, 0x1, 23) | \
                    ...
                    AI_BUFFER_FMT_PACK(0, 0x7, 14) | AI_BUFFER_FMT_SET_BITS(bits_) | AI_BUFFER_FMT_SET_FBITS(fbits_))
...
#define AI_BUFFER_META_INFO_INTQ_GET_SIZE(meta_) \
  ((AI_BUFFER_META_INFO_INTQ(meta_)) ? AI_INTQ_INFO_LIST_SIZE(AI_BUFFER_META_INFO_INTQ(meta_)) : 0)

#define AI_BUFFER_META_INFO_INTQ_GET_SCALE(meta_, pos_) \
  ((AI_BUFFER_META_INFO_INTQ(meta_)) ? AI_INTQ_INFO_LIST_SCALE(AI_BUFFER_META_INFO_INTQ(meta_), ai_float, pos_) : 0)
...
#define AI_BUFFER_META_INFO_INIT(flags_, intq_info_) {.flags = (flags_), .intq_info = AI_PACK(intq_info_)}
...
#define AI_BUFFER_SHAPE_ELEM(buf_, pos_) (((pos_) < AI_BUFFER_SHAPE_SIZE(buf_)) ? (buf_)->shape.data[pos_] : 0)
...
#define AI_BUFFER_ARRAY_OBJ_INIT_STATIC(flags_, size_, ...) \
  {.flags = (ai_u16)(flags_), .size = (ai_u16)(size_), .buffer = (ai_buffer*)((ai_buffer[]){__VA_ARGS__})}

#define AI_BUFFER_ARRAY_SANE(buf_array_) ai_buffer_array_sane(buf_array_)

#define AI_BUFFER_ARRAY_FLAGS(buf_array_) ((AI_BUFFER_ARRAY_SANE(buf_array_)) ? (buf_array_)->flags : AI_FLAG_NONE)

#define AI_BUFFER_ARRAY_SIZE(buf_array_) ((AI_BUFFER_ARRAY_SANE(buf_array_)) ? (buf_array_)->size : 0)
...
/*****************************************************************************/
#define AI_ERROR_INIT(type_, code_) {.type = AI_ERROR_##type_, .code = AI_ERROR_CODE_##code_}
 ...
 */
typedef enum { AI_RESIZE_ZEROS = 0x0, AI_RESIZE_NEAREST, AI_RESIZE_LINEAR, AI_RESIZE_CUBIC } ai_resize_mode;
...
typedef enum { AI_ROUND_PREFER_FLOOR = 0x0, AI_ROUND_PREFER_CEIL, AI_ROUND_FLOOR, AI_ROUND_CEIL } ai_nearest_mode;
 ...
 */
AI_API_ENTRY
ai_bool ai_buffer_array_item_set_address(ai_buffer_array* barray, const ai_u32 pos, ai_handle address);
