...
#define AI_SHAPE_GET_ELEM(shape_, pos_) (((pos_) < AI_SHAPE_SIZE(shape_)) ? AI_SHAPE_ELEM(shape_, pos_) : 1)
...
#define AI_SHAPE_CLONE(dst_, src_) AI_STORAGE_KLASS_COPY(dst_, ai_shape_dimension, src_, ai_shape_dimension)
...
#define AI_SHAPE_D(shape_) ((AI_SHAPE_SIZE((shape_)) > AI_SHAPE_DEPTH) ? AI_SHAPE_ELEM((shape_), AI_SHAPE_DEPTH) : 1)
#define AI_SHAPE_E(shape_) \
  ((AI_SHAPE_SIZE((shape_)) > AI_SHAPE_EXTENSION) ? AI_SHAPE_ELEM((shape_), AI_SHAPE_EXTENSION) : 1)
...
#define AI_STRIDE_GET_ELEM(stride_, pos_) (((pos_) < AI_STRIDE_SIZE(stride_)) ? AI_STRIDE_ELEM(stride_, pos_) : 0)
...
#define AI_STRIDE_CLONE(dst_, src_) AI_STORAGE_KLASS_COPY(dst_, ai_stride_dimension, src_, ai_stride_dimension)
...
#define AI_STRIDE_SET_D(stride, val) \
  if (AI_STRIDE_SIZE((stride)) >= 5) AI_STRIDE_SET_ELEM((stride), AI_SHAPE_DEPTH, val)
#define AI_STRIDE_SET_E(stride, val) \
  if (AI_STRIDE_SIZE((stride)) == 6) AI_STRIDE_SET_ELEM((stride), AI_SHAPE_EXTENSION, val)
...
#define AI_OFFSET_5D(y, x, d, e, ch, stride)                                                                           \
  (AI_OFFSET_H((y), (stride)) + AI_OFFSET_W((x), (stride)) + AI_OFFSET_D((d), (stride)) + AI_OFFSET_E((e), (stride)) + \
...
ai_bool ai_shape_is_same(const ai_shape* shape0, const ai_shape* shape1) {
  ...
  if (AI_SHAPE_SIZE(shape0) != AI_SHAPE_SIZE(shape1)) return false;
  ai_size dim = AI_SHAPE_SIZE(shape0);
  while (dim > 0) {
    dim--;
    if (AI_SHAPE_ELEM(shape0, dim) != AI_SHAPE_ELEM(shape1, dim)) return false;
  }
  return true;
}
...
ai_bool ai_shape_is_scalar(const ai_shape* shape0) {
  ...
  while (dim > 0) {
    dim--;
    if (AI_SHAPE_ELEM(shape0, dim) != 1) return false;
  }
  return true;
}
